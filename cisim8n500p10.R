...
simRep <- function(n = 250, p = 10)
{
  ...
  grf_fit_multi <- multi_regression_forest(x, Y = cbind(a, y),  num.trees = 1000,
                                           min.node.size = 10)
...
for (r in 1:nreps)
{
  ...
  {
    res_mat81 <- rbind(res_mat81, res_tmp)
  }
}
write.table(res_mat81,"resmat81",row.names = F,quote = F,col.names = TRUE) 